...
class Ganon extends Enemy {
  ...
  spawnFire() {
    ...
    if (Date.now() - this.oldTime > 1000) {
      let newFire = new Fire(this.canvas, this.ctx, [this.position[0] + this.width*this.scale, this.position[1]], [Math.random()*this.canvas.width, Math.random()*this.canvas.height]);
